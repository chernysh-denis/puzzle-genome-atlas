...
7. Derive the genome signature and apply the
   [canonical exact/near-match rules](docs/ARCHITECTURE.md#genome-signature)
   to the complete game index.
8. Test verified combination gene sets against the genome and compare only the
   mathematically selected near matches in detailed prose.
...
Every substantive claim follows the single canonical
[Evidence Model](docs/EVIDENCE_MODEL.md), using inline notation or a claim
ledger. Sources or reproduced evidence remain mandatory.
...
- [ ] The complete indexed corpus was scanned with the canonical formula.
- [ ] Exact matches and mathematically selected near matches are identified.
...
- [ ] A negative-result record is linked when a concrete prior claim,
      candidate or gene distinction was rejected; otherwise the analysis says
